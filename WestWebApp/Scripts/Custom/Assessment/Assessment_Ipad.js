function clearAssessment() {
    $('.assessment-indicator-postfix').empty();
}
...
function drawAssessment(jsondata) {
    ...
    $.each(jsondata.RESULTS, function () {
        if (indicatorDisplayed.indexOf(this.FLD) > -1) {
            ...
            var toConcat = ' ' + AssessmentText[0][this.FLD] + ' ';
            ...
            if (indicatorParameters[this.FLD].options && indicatorParameters[this.FLD].options.meter && indicatorParameters[this.FLD].options.meter.style == 'rgr_meter') {
                ...
                if (value <= 70 && value >= 30) {
                    goodStatement += toConcat + AssessmentText[3][this.FLD];
                    goodCount++;
                }
                else {
                    if (value < 30) {
                        toConcat += AssessmentText[1][this.FLD];
                    }
                    else {
                        toConcat += AssessmentText[5][this.FLD];
                    }

                    badStatement += toConcat;
                    badCount++;
                }
            }
            else {
                ...
                toConcat += AssessmentText[textIndex][this.FLD];

                if (value >= 60) {
                    goodStatement += toConcat;
                    goodCount++;
                }
                else {
                    badStatement += toConcat;
        ...
        if (AssessmentFields.indexOf(this.FLD) > -1) {
            ...
            statusText = AssessmentFieldBasePhrases[this.FLD] + ' ' + phrases[textIndex];

            if ('SAI_P' != this.FLD) {
                if (value >= ranges[1]) {
                    goodStatement = statusText + ' ' + goodStatement;
                    goodCount++;
                }
                else {
                    badStatement = statusText + ' ' + badStatement;
                    badCount++;
                }
            }
    ...
    if (goodCount) {
        $('#assessment-good .assessment-indicator-postfix').text(goodStatement);
    }

    if (badCount) {
        $('#assessment-bad .assessment-indicator-postfix').text(badStatement);
    }

    console.log(goodCount, goodStatement, badCount, badStatement);
}
