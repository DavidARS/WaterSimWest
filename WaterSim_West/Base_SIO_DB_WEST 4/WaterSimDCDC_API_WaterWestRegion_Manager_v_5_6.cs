...
namespace WaterSimDCDC
{
    ...
    // 
    //             6/9/19   Quay, Added Colorado River resource
    // 
    // ver 7.0     9/10/20  QUAY Added External Model Support
    //                      DAS Created Colorado River System Model
    //                      QUAY Added Initial links for Colorado River System Model to West/Basin Model
    //                      note: "WSW.7.5.0"; was hardcoded below
    //              
    // ver 7.5.1   09.16.20 Sampson. A) Add a version tag (WSB 7.5.1) on ~line 595 below. 
    //                      B) Add the objects for the three models: COriverModel FCORiverModel;
    ...
    //                      E) Added Desal water
    //                      F) Utah pipeline water transfers (not complete- DCP work still needs being comleted)
    //                      G) NewWaterSavings as they influence (reduce) water demand
    //                      H) Stormwater Potential (gal acre-1 day-1)
    //=============================================================================================================================================================================================
    // Enums 
    //==========================================================
    //
    ...
    // EDIT QUAY 9/8/20
    // OUCH!!! These are in the wrong order, I fixed this
    ...
    public static partial class ProviderClass
    {
        ...
        // EDIT QUAY 9/8/20
        // WHOOPS! Changed order have to change this as well
        
        // public const eProvider LastProvider = eProvider.eWyomingSouthwest;
        public const eProvider LastProvider = eProvider.eWyomingNotInBasin;
        ...
        public const eProvider FirstProvider = eProvider.eArizonaCentralSouth;
        ...
        public static eProvider[] BasinProviders = new eProvider[] {
            ...
            eProvider.eCaliforniaSouthwest, eProvider.eCaliforniaSoutheast,  
            eProvider.eColoradoFrontRange,eProvider.eColoradoInBasin,
            eProvider.eNevadaSouth, eProvider.eNevadaInBasin, 
            eProvider.eNewMexicoCentral, eProvider.eNewMexicoInBasin, eProvider.eNewMexicoGila,
            eProvider.eUtahSaltLake, eProvider.eUtahInBasin, 
            eProvider.eWyomingSouthwest, eProvider.eWyomingInBasin       };


        // EDIT QUAY 9/8/20
        // Added Lower Basin

        public static eProvider[] LowerBasinProviders = new eProvider[] {
            eProvider.eArizonaCentralSouth,  eProvider.eArizonaWest, eProvider.eArizonaNorth, eProvider.eArizonaSoutheast,eProvider.eArizonaCentralNorth,
            eProvider.eCaliforniaSouthwest, eProvider.eCaliforniaSoutheast, 
            eProvider.eNevadaSouth, eProvider.eNevadaInBasin,
            eProvider.eNewMexicoGila
            };
        // END EDIT 9/8/20
...
// NOTE the regional weight is based on parameter #3
        ...
        // edits 01.31.22 das
       public const int epP_UrbanLowDensityLSFManagement  = 200;
       public const int epP_UrbanLowDensityTSFManagement  = 201;
       public const int epP_UrbanLowDensitySSFManagement  = 202;
       public const int epP_UrbanHighDensitySMFManagement = 203;
       public const int epP_UrbanHighDensityWMFManagement = 204;
       public const int epP_UrbanHighDensityMMFManagement = 205;
       public const int epP_UrbanHighDensityHMFManagement = 206;

       public const int epP_SuburbanDensityManagement     = 207;
       public const int epP_ExurbanHighDensityManagement  = 208;
       public const int epP_ExurbanLowDensityManagement   = 209;
        // end edits 01.31.22 das
...
// END EDIT
        ...
        // EDIT QUAY 9/8/12
        // Added values for External Model Control 
        public const int epP_SURFACE_USE_EXT_MODEL = 1930;  // Use Surface external model
        public const int epP_COLORADO_USE_EXT_MODEL = 1931;  // Use Colorado River external model
        public const int epP_RECLAIMED_USE_EXT_MODEL = 1932;  // Use Reclaimed water external model
        public const int epP_GROUNDWATER_USE_EXT_MODEL = 1933;  // Use Groundwater external model
        public const int epP_LAKEWATER_USE_EXT_MODEL = 1934;  // Use Lake water external model
        public const int epP_SALINEWATER_USE_EXT_MODEL = 1935;  // Use Saline/Ocean water external model
        public const int epP_GRAYWATER_USE_EXT_MODEL = 1936;  // Use Graywater external model

        public const int epP_StreamAllocation = 1940;
        public const int ep_StreamWithdraw = 1941;
        public const int ep_ColoradoRiverTraceStartYear = 1942;
    ...
    public partial class WaterSimManager :  WaterSimManagerClass
    {
      ...
      //  WaterSimDCDC.Processes.AlterWaterManagementFeedbackProcess WM;
        ...
        //
        // 09.15.20 das
        // We had three different lines where the stop year was being set. I wanted to bring it to one
        int StopYear
        {
            get {return DefaultStopYear(); } 
        }
        // I need to pass this into the Colorado River model-reservoir operations- so that the object
        // knows how long to run the empirical data through the river reservoirs.
        /// <summary>
        /// Self explanatory
        /// </summary>
        ...
        protected override void initialize_ModelParameters()
        {
          ...
          //     ExtendDoc.Add(new WaterSimDescripItem(eModelParam.epPolicyStartYear, "Year that the Policies are implemented", "yr", "Year", "", new string[] { }, new int[] { }, new ModelParameterGroupClass[] { }));
            ...
            // ======================================================
            #region demand conservation and water management
            // ICLUS I target reductions
            ...
            // edits 01.31.22 das NEW parameters for added density classes
            // ===============================================================================

            // End edits 09.20.21 das
            //      edits 02.01 22 das
            // Using Denver water density classes
            // SMF - small multi family, WMF - Three story walk-up, MMF- Medium Multi-family, HMF - High density Multi-Family
            // Associated DUA are found elsewhere
            ...
            // 09.15.20 das - not used yet
            // This parameter accesses the array of regional stream allocation
            //WestModel.ColoradoRiverModel.StreamAllocation = new providerArrayProperty(_pm, eModelParam.epP_StreamAllocation, WestModel.ColoradoRiverModel.Geti_StreamAllocation, eProviderAggregateMode.agSum);
            //_pm.AddParameter(new ModelParameterClass(eModelParam.epP_StreamAllocation, "Regional Stream Allocation", "RSTALLOC", rangeChecktype.rctNoRangeCheck, 0, 0, null, WestModel.ColoradoRiverModel.StreamAllocation));

            //// This parameter sets or get value from the stream model
            //// NOTE!! Add some ExtendDoc information
            //// [09.11.20 das - the getter was causing an exception]
            //_pm.AddParameter(new ModelParameterClass(eModelParam.ep_StreamWithdraw, "Stream Withdrawal", "STRWD", rangeChecktype.rctCheckRange, 0, 100000000, WestModel.ColoradoRiverModel.Geti_StreamWithdraw, WestModel.ColoradoRiverModel.Seti_StreamWithdraw, null));
            // end 09.15.20 das

            // Sets the Start year for the Colorado River Trace
            _pm.AddParameter(new ModelParameterClass(eModelParam.ep_ColoradoRiverTraceStartYear, "CO River Trace Year", "CO_STRT", rangeChecktype.rctCheckRange, 1906, 1988, WestModel.ColoradoRiverModel.Geti_CoRiverTraceStartYear, WestModel.ColoradoRiverModel.Seti_CoRiverTraceStartYear, null));
            // 09.15.20 das
...
// QUAY EDIT 9/13/18
            ...
            #endregion
            // =================================================================================
            #region Demand model to use (index) - sets the water demand model(s) to use

            // Demand Model Parameters
            // Last Write: 06.01.18, 02.16.21, 02.21.22
            // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            // Demand Model to Use
            //
            // Urban: 1 = GPCD , 2 = Land-cover land-use, 3 = LCLU from ICLUS II
            ...
            _pm.AddParameter(new ModelParameterClass(eModelParam.epP_DM_MODELtoUSE_Urban_P, "Urban Demand Model: 1=GPCD, 2=ICLUS I, 3=ICLUS II", "DEMMODU_P", rangeChecktype.rctCheckRange, 0, 5, RangeCheck.NoSpecialProvider, WestModel.DemandModelUrban_Index));
            ...
            _pm.AddParameter(new ModelParameterClass(eModelParam.epP_DM_MODELtoUSE_Ag_P, "Ag Demand Model to Use", "DEMMODA_P", rangeChecktype.rctCheckRange, 0, 5, RangeCheck.NoSpecialProvider, WestModel.DemandModelAg_Index));
            ...
            _pm.AddParameter(new ModelParameterClass(eModelParam.epP_DM_MODELtoUSE_Ind_P, "Ind Demand Model to Use", "DEMMODI_P", rangeChecktype.rctCheckRange, 0, 5, RangeCheck.NoSpecialProvider, WestModel.DemandModelInd_Index));
        ...
        #region Updated Policies October 2021 - Utah pipeline, rainwater harvesting, and atmospheric water Gets and Sets
        // ===============================================
        ...
        void DefaultSettings()
        {
            ...
            // 09.16.20 das appears to work correctly
            //WestModel.ColoradoRiverModel.Seti_CoRiverTraceStartYear(1952);
            // 09.16.20 end
            // 09.17.20 das test the drought function
            WestModel.DroughtActive[12] = 1;
            WestModel.DroughtActive[18] = 1;
            //WestModel.DroughtControl[12] = 30;
           // WestModel.DroughtControl[18] = 40;
            WestModel.DroughtDepth[12] = 70;
            WestModel.DroughtDepth[18] = 80;
            WestModel.DroughtStartYear[12] = 2021;
            WestModel.DroughtStartYear[18] = 2025;
            WestModel.DroughtLength[12] = 15;
            WestModel.DroughtLength[18] = 20;
            // 09.17.20 das end
            ...
            WestModel.ColoradoRiverModel.WSMstopYear = StopYear;
            // 09.16.20 end
            // ====================================
        }
