...
// Copied DAS code in then modified it in external models below, These models are now in WaterWim_CORiverModel class
        ...
        SurfaceModel FSurfaceModel;
        WaterSim_CORiverModel FColoradoModel;
        ...
        public WaterSimModel(string DataDirectoryName, string TempDirectoryName)
        {
            ...
            try
            {
                ...
                FColoradoModel = new WaterSim_CORiverModel(DataDirectoryName, TempDirectoryName, UnitDataFielname);
                ...
                // 09.14.20 das - Where does this go?
                //FColoradoModel.ColoradoRiverModel.RunCOoneYear(year)
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public virtual int runOneYear(int year)
        {
            ...
            foreach (ResourceModel RM in FResourceModels)
            {
               if (RM is SurfaceModel)
               {
                    int errCode = 0;
                    string errStr = "";
                    (RM as SurfaceModel).RunYear(out errCode,out errStr);
               }
                if (RM is WaterSim_CORiverModel)
                {
                    int errCode = 0;
                    string errStr = "";
                    // 09.15.20 das
                    (RM as WaterSim_CORiverModel).RunYear(year, out errCode, out errStr);
                    // end 09.15.20 das
                }
            }
            // END MODIFIED Quay 9/8/2020
